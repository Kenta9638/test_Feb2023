function [X,Y,Z] = scatter2Pcolor(x,y,z,varargin)
...
% Author: Etienne Cheynet  - last modified : 10/05/2016
...
p.addOptional('resolution',10000);  % <----- !!!
...
% check nan values for x
if any(isnan(x)),
    indX_NaN = find(isnan(x));
    warning('The coordinates  x of the input points contains NaN values.');
else
    indX_NaN = [];
end

% check inf values for x
if any(isinf(x)),
    indX_Inf = find(isinf(x));
    warning('The coordinates x of the input points contains Inf values.');
else
    indX_Inf = [];
end

% check nan values for y
if any(isnan(y)),
    indY_NaN = find(isnan(y));
    warning('The coordinates y of the input points contains NaN values.');
else
    indY_NaN = [];
end
% check inf values for y
if any(isinf(y)),
    indY_Inf = find(isinf(y));
    warning('The coordinates y of the input points contains Inf values.');
else
    indY_Inf = [];
end

% remove inf and nan values
indTot = unique([indX_NaN(:);indX_Inf(:);indY_NaN(:);indY_Inf(:)]);
if ~isempty(indTot),
    x(indTot)=[];
    y(indTot)=[];
    z(indTot)=[];
