function [Roystest,W,P,Z,R,NC,e,T,H] = Roystest(X,dim,alpha)
...
% KKUROSAWA
if p ~= dim
  disp('error in Roystest!!')
  pause
end
flag1 = zeros(1,p);
for i = 1:p
  if abs(mean(X(:,i))-min(X(:,i))) == 0.000 && abs(mean(X(:,i))-max(X(:,i))) == 0.0000
    flag1(i) = 1;
  end
end
%tmp_num = (0.96-0.93)/(300-40)*(n-40)+0.93; % (40mem:0.93, 300mem:0.96)
...
elseif (n >= 4) && (n <=11),
    ...
    for j = 1:p,
        W(j) = ShaWilstat(X(:,j),flag1(j));
    %    [tmp_Roys(j),W(j)] = swtest(X(:,j),allzero_flag(j)); % KKUROSAWA
...
for j = 1:p,
 %   R(j) = (norminv((normcdf( - Z(j)))/2))^2;
    tmp_var1 = normcdf(-Z(j));
  %  tmp_var1 = normcdf(Z(j));
    tmp_var2 = tmp_var1/2;
    tmp_var3 = norminv(tmp_var2);
    tmp_var4 = tmp_var3^2;
    R(j)     = tmp_var4;
end
%R
...
u = 0.715;
v = 0.21364 + 0.015124*(log(n))^2 - 0.0018034*(log(n))^3; % Royston 1983
l = 5;
...
%l
%C
%if C(1,2) > 0.90 && mean(W) > 0.90 % KKUROSAWA
%  P = alpha;
%else
