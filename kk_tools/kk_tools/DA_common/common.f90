...
SUBROUTINE com_nanmean(ndim,var,amean)
  IMPLICIT NONE
  ...
  counter = 0
  DO i=1,ndim
    IF (abs(var(i))<99.9) THEN
      counter = counter + 1
      amean = amean + var(i)
    ENDIF
  END DO
  amean = amean / REAL(counter,r_size)

  RETURN
END SUBROUTINE com_nanmean
!-----------------------------------------------------------------------
! Standard deviation
!-----------------------------------------------------------------------
...
! Spread ! KK
!-----------------------------------------------------------------------
SUBROUTINE com_sprd(ndim,nmem,var,aout)
  IMPLICIT NONE

  INTEGER,INTENT(IN) :: ndim,nmem
  REAL(r_size),INTENT(IN) :: var(ndim,nmem)
  REAL(r_size),INTENT(OUT) :: aout

  REAL(r_size) :: amean(ndim)
  REAL(r_size) :: dx(ndim,nmem)
  REAL(r_size) :: sum1
  INTEGER :: i,j

  
  DO i = 1,ndim
    CALL com_mean(nmem,var(i,:),amean(i))
  ENDDO

  sum1 = 0.0d0
  DO i = 1,ndim
    DO j = 1,nmem
      dx(i,j) = var(i,j) - amean(i)
      sum1 = sum1 + dx(i,j)**2
    ENDDO
  ENDDO

  aout = SQRT( sum1 / REAL(ndim*nmem,r_size) )

  RETURN
END SUBROUTINE com_sprd
!-----------------------------------------------------------------------
! Covariance
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
! RANDPERM in MATLAB (by KK)
!-----------------------------------------------------------------------
SUBROUTINE com_randperm(ndim,selectnum,var)
  IMPLICIT NONE

  INTEGER,INTENT(IN)  :: ndim,selectnum
  INTEGER,INTENT(OUT) :: var(1:selectnum)
  INTEGER,ALLOCATABLE :: tmp_selected(:)
  ...
  ALLOCATE(tmp_selected(ndim)); tmp_selected=0
  ALLOCATE(tmp_var(selectnum)); tmp_var=0

  DO i=1,selectnum
    DO
      CALL RANDOM_NUMBER(rnd)
      tmp_num=int(rnd*ndim)+1
      IF (tmp_selected(tmp_num)==0) THEN
        tmp_var(i) = tmp_num
        EXIT
      ENDIF
    ENDDO
    tmp_selected(tmp_num)=9999
  ENDDO
  
  DO i=1,selectnum
    var(i) = minval(tmp_var)
    tmp_var(minloc(tmp_var))=9999
  ENDDO
END SUBROUTINE com_randperm
!-----------------------------------------------------------------------
! TIMEINC
!-----------------------------------------------------------------------
