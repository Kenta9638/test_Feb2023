function h = cbarrow(options,kk_colormap,sharpness,c_in,str_color,end_color)
...
% Plot arrows: 
if makerightarrow 
    rightarrowx = (cbpos(1)+cbpos(3)) + [0 cbpos(4)*sqrt(2)/sharpness 0 0]; 
    rightarrowy = cbpos(2) + [0 cbpos(4)/2 cbpos(4) 0]; 
    hr = patch(rightarrowx,rightarrowy,end_color,'EdgeColor','k','linewidth',1.1); 
end

if makeleftarrow
    leftarrowx = cbpos(1) + [0 -cbpos(4)*sqrt(2)/sharpness 0 0]; 
    leftarrowy = cbpos(2) + [0 cbpos(4)/2 cbpos(4) 0]; 
    hl = patch(leftarrowx,leftarrowy,str_color,'EdgeColor','k','linewidth',1.1); 
end

if makeuparrow
    uparrowx = cbpos(1) + [0 cbpos(3) cbpos(3)/2 0]; 
    uparrowy = cbpos(2)+cbpos(4) + [0 0 cbpos(3)*sqrt(2)/sharpness 0];
    hu = patch(uparrowx,uparrowy,end_color,'EdgeColor','k','linewidth',1.1); 
end

if makedownarrow
    downarrowx = cbpos(1) + [0 cbpos(3) cbpos(3)/2 0]; 
    downarrowy = cbpos(2) + [0 0 -cbpos(3)*sqrt(2)/sharpness 0]; 
    hd = patch(downarrowx,downarrowy,str_color,'EdgeColor','k','linewidth',1.1); 
end
...
if strcmpi(cb.Box,'on')
    ...
    if all([makerightarrow makeleftarrow])
        line(cbpos(1) +[0 cbpos(3) cbpos(3)+cbpos(4)*sqrt(2)/sharpness cbpos(3) 0 -cbpos(4)*sqrt(2)/sharpness 0],...
    ...
    if all([makerightarrow ~makeleftarrow])
        line(cbpos(1) +[0 cbpos(3) cbpos(3)+cbpos(4)*sqrt(2)/sharpness cbpos(3) 0 0],...
    ...
    if all([~makerightarrow makeleftarrow])
        line(cbpos(1) +[0 cbpos(3) cbpos(3) 0 -cbpos(4)*sqrt(2)/sharpness 0],...
    ...
    if all([makeuparrow makedownarrow])
        line(cbpos(1) +[0 0 cbpos(3)/2 cbpos(3) cbpos(3) cbpos(3)/2 0],...
            cbpos(2) + [0 cbpos(4) cbpos(4)+cbpos(3)*sqrt(2)/sharpness cbpos(4) 0 -cbpos(3)*sqrt(2)/sharpness 0],...
            'color',linecolor,'LineWidth',linewidth)
    ...
    if all([makeuparrow ~makedownarrow])
        line(cbpos(1) +[0 0 cbpos(3)/2 cbpos(3) cbpos(3) 0],...
            cbpos(2) + [0 cbpos(4) cbpos(4)+cbpos(3)*sqrt(2)/sharpness cbpos(4) 0 0],...
            'color',linecolor,'LineWidth',linewidth)
    end
    ...
    if all([~makeuparrow makedownarrow])
        line(cbpos(1) +[0 0 cbpos(3) cbpos(3) cbpos(3)/2 0],...
            cbpos(2) + [0 cbpos(4) cbpos(4) 0 -cbpos(3)*sqrt(2)/sharpness 0],...
            'color',linecolor,'LineWidth',linewidth)
    end
end
