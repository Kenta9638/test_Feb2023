...
!-----------------------------------------------------------------------
! Quick sort; Kenta Kurosawa; Nov.17,2022
!-----------------------------------------------------------------------
SUBROUTINE kk_qsort(ndim,inp_var,out_var,out_num)
IMPLICIT NONE

INTEGER,      INTENT(IN)  :: ndim
REAL(r_size), INTENT(IN)  :: inp_var(ndim)
REAL(r_size), INTENT(OUT) :: out_var(ndim)
INTEGER,      INTENT(OUT) :: out_num(ndim)
INTEGER :: ii, jj, tmp_num
REAL(r_size) :: tmp_var, var(ndim), tmp_UNDEF

  var        = inp_var
  tmp_UNDEF  = minval(var) - dble(9999)
  out_var(:) = UNDEF
  out_num(:) = int(UNDEF)
  DO ii = 1,ndim
    tmp_num = maxval(maxloc(var))
    tmp_var = maxval(var)
    IF (var(tmp_num) /= tmp_var) WRITE(*,*) 'WARNING1 from kk_qsort !!'
    out_var(ii)  = tmp_var
    out_num(ii)  = tmp_num
    var(tmp_num) = tmp_UNDEF
  ENDDO

  ! check
  IF (maxval(inp_var) /= maxval(out_var)) WRITE(*,*) 'WARNING2 from kk_qsort !!'
  IF (minval(inp_var) /= minval(out_var)) WRITE(*,*) 'WARNING3 from kk_qsort !!'
  IF (maxval(out_num) /= ndim)            WRITE(*,*) 'WARNING4 from kk_qsort !!'
  IF (minval(out_num) /= 1)               WRITE(*,*) 'WARNING5 from kk_qsort !!'

END SUBROUTINE kk_qsort
