...
!
! Feb.2023: Adaptive hybrid PF-EnKF (Kurosawa and Poterjoy 2023; MWR) by Kenta Kurosawa 
!
...
real(r8) :: orig_obs_prior(ens_size), max_res
real(r8) :: min_res(ens_handle%my_num_vars), min_res_y(obs_ens_handle%my_num_vars) ! KKUROSAWA
...
REAL(r_size) :: tmp_roys_mdl_r_size(ens_handle%my_num_vars)
REAL(r_size) :: tmp_roys_obs_r_size(obs_ens_handle%my_num_vars)
real(r8), parameter :: SW_alpha_mdl = 0.050d0
real(r8), parameter :: SW_alpha_obs = 0.050d0
!- min residual
real(r8)            :: out_min_res(ens_handle%my_num_vars)
real(r8)            :: out_min_res_y(obs_ens_handle%my_num_vars) 
real(r8)            :: global_tmp_min_res(ens_handle%num_vars)
real(r8)            :: global_tmp_min_res_y(obs_ens_handle%num_vars)
real(r8)            :: global_out_min_res(ens_handle%num_vars)
real(r8)            :: global_out_min_res_y(obs_ens_handle%num_vars)
!- counter (LPF iteration)
INTEGER             :: out_cnt(ens_handle%my_num_vars)
INTEGER             :: out_cnt_y(obs_ens_handle%my_num_vars)  
INTEGER             :: global_tmp_cnt(ens_handle%num_vars) 
INTEGER             :: global_tmp_cnt_y(obs_ens_handle%num_vars)  
INTEGER             :: global_out_cnt(ens_handle%num_vars) 
INTEGER             :: global_out_cnt_y(obs_ens_handle%num_vars)  
integer             :: tmp_num1, tmp_num2
!- DA
real(r8)            :: obs_err_var_org
...
if (filter_kind == 9) then
  ...
  if (pf_enkf_hybrid) then
     ...
     min_res_y = min_residual

     IF (adaptive_minres_flag) THEN ! KKUROSAWA
       out_min_res   = UNDEF ! -Infinity
       out_min_res_y = UNDEF ! -Infinity
       out_cnt       = int(UNDEF)
       out_cnt_y     = int(UNDEF)
     ELSE
       out_min_res   = min_res
       out_min_res_y = min_res_y
     ENDIF
  else
     min_res   = 0.0_r8
     min_res_y = 0.0_r8
...
ITERATIONS: do iter = 1,maxiter
    ...
    ELSE ! last iteration (EnKF update)
      ...
      DO i = 1, obs_ens_handle%num_vars
        ...
        IF (obs_ens_handle%my_pe == owner) THEN
          global_tmp_min_res_y(i) = out_min_res_y(owners_index) 
      ...
      if(ens_handle%my_pe == owner) then
        ...
        if(nint(obs_qc) == 0) then
          ...
          if (1.0_r8 > ens_size * 0.98_r8 *sum(hw**2) ) then
            ...
            call pf_regularization_minw(d/temp1, 1E-10_r8 / ens_size, ens_size, obs_err_infl) ! 195:find_obs_infl
  ...
  SEQUENTIAL_OBS: do i = 1, obs_ens_handle%num_vars
   ...
   base_obs_loc = get_obs_def_location(obs_def)
   obs_err_var = get_obs_def_error_variance(obs_def)
   obs_err_var_org = obs_err_var ! KKUROSAWA
         ...
         do group = 1, num_groups
            ...
            if(local_varying_ss_inflate .and. varying_ss_inflate > 0.0_r8 .and. &
               ...
               IF (filter_kind_orig == 9 .AND. adaptive_minres_flag) THEN ! KKUROSAWA
                 obs_err_infl = 1.0_r8 / max(out_min_res(state_index),0.00010d0) 
...
IF (adaptive_minres_flag) THEN
  ...
  DO i = 1, ens_handle%num_vars
    CALL get_var_owner_index(ens_handle, int(i,i8), owner, owners_index)
    IF (ens_handle%my_pe == owner) THEN
      ens_handle%copies(tmp_num1,owners_index) = global_tmp_cnt(i) ! ENS_MEAN_COPY+1
      ens_handle%copies(tmp_num2,owners_index) = global_tmp_min_res(i)     ! ENS_SD_COPY+1
    ENDIF
  ENDDO
...
! KKUROSAWA test
subroutine update_from_obs_inc_tempered(obs, obs_prior_mean, obs_prior_var, obs_inc, &
               state, ens_size, min_res, state_inc, reg_coef, net_a, correl_out)
!========================================================================
...
real(r8),           intent(in)    :: obs_prior_mean, obs_prior_var
real(r8),           intent(in)    :: state(ens_size)
real(r8),           intent(in)    :: min_res ! KKUROSAWA
real(r8),           intent(out)   :: state_inc(ens_size), reg_coef
real(r8),           intent(inout) :: net_a
real(r8), optional, intent(inout) :: correl_out
...
if (obs_prior_var > 0.0_r8) then
!   reg_coef = obs_state_cov/obs_prior_var
   reg_coef = obs_state_cov/obs_prior_var*min_res ! KKUROSAWA
else
   reg_coef = 0.0_r8
endif
...
end subroutine update_from_obs_inc_tempered
